...
- 平衡运行：选择期初和期末有效快照，在同一事务内锁定储罐并重新读取边界快照和物理转移；仅完全落入期间的已确认转移计入，任一已确认转移跨越期间起点或终点即整体拒绝并返回转移编号和越界时刻，草稿与取消记录不影响结果，保存完整输入、系数版本、方程和不确定度证据。
...
- `TRANSFER_CROSSES_PERIOD_BOUNDARY`：存在已确认转移跨越期间起点或终点，平衡整体拒绝，不剔除也不折算；错误 `details.violations` 返回转移编号、跨越边界和越界时刻，页面逐条展示。草稿与取消记录不参与判定。
